...
description: >-
  Contentstack Developer Hub apps, manifests, GraphQL, and HTTP helpers for the
  Apps CLI plugin. Use when changing API calls, manifest handling, regions, or
  shared app types in packages/contentstack-apps-cli.
---

# Contentstack Apps domain – Apps CLI plugin
...
Source lives under **`packages/contentstack-apps-cli/`** in the [cli-plugins](https://github.com/contentstack/cli-plugins) monorepo.

## Instructions

### Configuration

- **`packages/contentstack-apps-cli/src/config/index.ts`** — Default filenames, Developer Hub base URL defaults, and related constants consumed by commands and `BaseCommand`.
