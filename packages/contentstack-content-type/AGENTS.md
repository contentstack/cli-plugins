...
| Field | Detail |
|-------|--------|
| **Name:** | `contentstack-cli-content-type` ([contentstack/cli-plugins](https://github.com/contentstack/cli-plugins) → `packages/contentstack-content-type`) |
...
|--------------|---------|
| Build (publishable) | `pnpm run build` |
| Test | `pnpm test` |
| Test + coverage | `pnpm run test:coverage` |
| Lint | `pnpm run lint` (or `eslint . --ext .ts --config .eslintrc`) |
