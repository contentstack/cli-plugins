...
|--------|----------|
| `pnpm test` | Runs Jest |
| `pnpm run test:unit` | Alias for Jest (used by monorepo CI) |
| `pnpm run test:coverage` | Jest with coverage (see [jest.config.js](../../jest.config.js)) |
| `pnpm run lint` | ESLint on `.ts` files per [package.json](../../package.json): `eslint . --ext .ts --config .eslintrc` |

When validating changes, run **`pnpm test`**; ensure **ESLint** still passes (`pnpm run lint`). Use **`pnpm run test:coverage`** when changing `src/core/` or `src/utils/` behavior.
From the monorepo root: `pnpm --filter contentstack-cli-content-type run test`.
...
- Run **`pnpm test`** before pushing; use **`pnpm run test:coverage`** when changing core logic to confirm coverage (see [jest.config.js](../../jest.config.js) thresholds and [AGENTS.md](../../AGENTS.md)).
