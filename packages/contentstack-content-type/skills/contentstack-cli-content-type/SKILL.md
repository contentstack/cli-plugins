...
npm package `contentstack-cli-content-type` — part of the [contentstack/cli-plugins](https://github.com/contentstack/cli-plugins) monorepo (`packages/contentstack-content-type`): a **Contentstack CLI** (`csdx`) plugin that reads Content Type metadata from a stack—list, field details, audit log lines, version or cross-stack comparison, and stack content-model diagrams.
...
`package.json` scripts **`build`** and **`version`** drive `tsc`, `oclif manifest`, and `oclif readme`. After changing commands, flags, or descriptions, keep **README.md** and **oclif.manifest.json** in sync by running **`pnpm run build`**—see [dev-workflow/SKILL.md](../dev-workflow/SKILL.md) for commands and workflow.
From the monorepo root: `pnpm --filter contentstack-cli-content-type run build`.
...
- [dev-workflow/SKILL.md](../dev-workflow/SKILL.md) — TypeScript build, ESLint, oclif docs, `pnpm run build`.
