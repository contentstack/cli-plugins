  ...
  async run() {
    ...
    try {
      ...
      if (config["global-field"]) {
        await updateContentTypeForGlobalField(
          stack,
          config["content-type"],
          config
        );
      } else {
        await updateSingleContentTypeEntries(
          stack,
          config["content-type"],
          config
        );
      ...
      if (
        config.errorEntriesUid &&
        Object.keys(config.errorEntriesUid).length > 0
      ) {
...
JsonMigrationCommand.flags = {
  ...
  alias: flags.string({
    char: "a",
    description:
      "Enter the alias name. You must use either the --alias flag or the --stack-api-key flag.",
    required: false,
  }),
  ...
  "content-type": flags.string({
    description:
      "Specify the UID of the content type for which you want to migrate HTML RTE content.",
    required: false,
  }),
  ...
  "html-path": flags.string({
    description:
      "Enter the path to the HTML RTE whose content you want to migrate.",
    dependsOn: ["json-path"],
    required: false,
  }),
  "json-path": flags.string({
    description:
      "Enter the path to the JSON RTE to which you want to migrate the HTML RTE content.",
    dependsOn: ["html-path"],
    required: false,
  }),
  delay: flags.integer({
