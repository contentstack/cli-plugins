...
describe("Migration Config validation", () => {
  ...
  fancy
    ...
    .it("throw error on config without alias property", async () => {
      ...
      expect(error.message).to.contain(
        'is not exactly one from "stack-api-key","alias"'
      );
    });
...
describe("Content Type with Single RTE Field of Single Type", function () {
  ...
  beforeEach(() => {
    ...
    nock(testApiUrl, {
      ...
      .query(true)
      .reply(200, function (uri) {
        let search = this.req.options.search || "";
    ...
    nock(testApiUrl, {
      ...
      .reply(200, () => {
        return {
          locales: [{ code: "en-in", localized: true }, { code: "en-us" }],
        };
      });
    ...
    nock(testApiUrl, {
      ...
      .reply(200, function (uri) {
        ...
        if (query.locale) {
          return getEntry(contentTypeUid, entryUid, query.locale);
        }
        return getEntry(contentTypeUid, entryUid);
      });
  ...
  fancy
    ...
    .stub(command, "getToken", getTokenCallback)
    .it(
      "throw error on migration of Mutiple Html rte with single Json rte",
      async () => {
        const { error } = await runCommand(
          [
            "cm:entries:migrate-html-rte",
  ...
  fancy
    ...
    .stub(command, "getToken", getTokenCallback)
    .it(
      "should have proper json structure for images migrated from HTML RTE",
      async () => {
        const { stdout } = await runCommand(
          [
            "cm:entries:migrate-html-rte",
        ...
        expect(stdout).to.match(
          /Updated \d+ Content Type\(s\) and \d+ Entrie\(s\)/
        );
      }
    );
});
...
describe("Global Field Migration", () => {
  ...
  fancy
    ...
    .stub(command, "getToken", getTokenCallback)
    .it(
      "throw error on global field with empty referred content_types",
      async () => {
        const { error } = await runCommand(
          [
            "cm:entries:migrate-html-rte",
  ...
  fancy
    ...
    .stub(command, "getToken", getTokenCallback)
    .it(
      "throw error on global field with empty schema content_type",
      async () => {
        const { error } = await runCommand(
          [
            "cm:entries:migrate-html-rte",
