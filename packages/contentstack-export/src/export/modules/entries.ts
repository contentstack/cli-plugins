...
export default class EntriesExport extends BaseClass {
  ...
  public exportVariantEntry = false;
  ...
  createRequestObjects(
    ...
    const requestObjects: Array<Record<string, any>> = [];
  ...
  async fetchEntriesVersions(
    entries: any,
    options: { contentType: string; locale: string; versionedEntryPath: string },
  ): Promise<void> {
    log.debug(`Fetching versions for ${entries.length} entries...`, this.exportConfig.context);

    const onSuccess = ({ apiData: entry, response }: any) => {
    ...
    const onReject = ({ apiData: { uid } = undefined, error }: any) => {
  ...
  async start() {
    try {
      ...
      for (const entryRequestOption of entryRequestOptions) {
        log.debug(
