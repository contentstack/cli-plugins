import { handleAndLogError, log, messageHandler, sanitizePath } from '@contentstack/cli-utilities';
...
export default class ExportTaxonomies extends BaseClass {
  ...
  private isLocaleBasedExportSupported = true;
  ...
  private qs: {
    ...
    locale?: string;
    skip: number;
  }; // Flag to track if locale-based export is supported
   ...
   */
  async fetchTaxonomies(localeCode?: string, checkLocaleSupport = false): Promise<void> {
    let skip = 0;
    const localeInfo = localeCode ? `for locale: ${localeCode}` : '';
