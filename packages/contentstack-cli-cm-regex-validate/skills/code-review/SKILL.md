...
- Confirm tests and lint pass locally (CI runs `pnpm test` only; see [`AGENTS.md`](../../AGENTS.md)); no secrets in commits; plugin `files` and `prepack` remain consistent; workflows aligned with Node version used in CI.
...
- [ ] ESLint passes (`pnpm run lint`). **Note:** CI does not run ESLint — only `pnpm test` in [`.github/workflows/unit-test.yml`](../../.github/workflows/unit-test.yml); lint must pass locally before merge.
...
- [ ] `build` script still runs `tsc`, `oclif manifest`, and `oclif readme` as needed for the plugin.
...
- [ ] Unit test workflow exercises `pnpm test` on a supported Node version (see `.github/workflows/unit-test.yml`). Expect Jest only there — not `posttest`/ESLint unless you add a separate workflow.
