...
- **Generation** is delegated to **`@contentstack/types-generator`** ([npm](https://www.npmjs.com/package/@contentstack/types-generator), [GitHub](https://github.com/contentstack/types-generator))—version **^3.10.0** in [package.json](../../package.json). Do not assume a local clone; use npm and published source only.
- Uses **v2** `@contentstack/cli-command` and `@contentstack/cli-utilities` (`~2.0.0-beta.*`).
...
- Commands compile to **`lib/commands`**; **`pnpm run build`** refreshes **`oclif.manifest.json`** and README.
