...
export default class Entries extends BaseClass {
  ...
  async run(totalCount?: number) {
    ...
    log.debug(`Asset metadata prepared: ${Object.keys(this.assetMetaData).length} assets found`, this.config.auditContext);
          ...
          for (const fileIndex in indexer) {
            ...
            for (const entryUid in this.entries) {
              ...
              if (this.missingAssetRefs[this.currentUid]?.length) {
                log.debug(
                  `Found ${this.missingAssetRefs[this.currentUid].length} missing/quarantined asset references for entry ${uid}`,
                  this.config.auditContext,
                );
      ...
      log.debug(`- Missing/quarantined asset references: ${Object.keys(this.missingAssetRefs).length}`, this.config.auditContext);
  ...
  removeEmptyVal() {
    ...
    log.debug(
      `Cleanup completed: removed ${removedRefs} empty refs, ${removedSelectFields} empty select fields, ${removedMandatoryFields} empty mandatory fields, ${removedAssetRefs} empty asset refs`,
      this.config.auditContext,
    );
  }
